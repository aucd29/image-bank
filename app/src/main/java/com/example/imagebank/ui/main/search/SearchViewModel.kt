 ...
 *
 * - 어느게 가장 우수하냐 라는 질문을 잘못 받아드려 여기 있는 사람들 중에 어느게 가장 우수한가로 잘못 인식하여
 *   답변을 제대로 못했다. (그런 질문을 할일은 없었겠지... =_ =) 되세김질을 하자면
 *   개발자가 편하게 비즈니스 코드에 집중할 수 있도록 베이스 코드를 설계하고 공통 모듈을 설계하는걸 가장 오래 해왔고
 *   가장 잘하는 부분이라고 하고 싶다. (개인적으로는 js, cpp, kotin 영역없이 코딩 가능 하다.)
 *
 * - 팀에 와서 무얼 할수 있겠냐? - 거즘 추천으로만 회사를 옮겨다니다 보니 정말 오랬만에 받은 질문이라 머랄까?
 * 멀 할수 있을까? 라는 문제와 지금에 팀 구성원을 모르는 문제 때문에 제대로 답변을 못했다. 그냥 인터뷰에서는
 * 질러도 될거 같긴한데 이놈에 성격 =_ = 문제다. js, cpp, java, kotlin 구분 없이 자유롭게 쓰는데
 * 뭐든 할수 있다고 해야 했나? 그렇다고 과장하긴 싫고 이건 다음 트라이전까지는 고민해볼 문제다.
 *
 * - 질문하고 싶은게 있나? 라고 하여
 ...
 *    앱을 실시간으로 해킹하는걸 봐온지라 살짝 위험할거 같기도 하다.. (해커들이 네트워크 중간단을 뒤지는게 아니고 뷰단을 뒤져서 메모리를 건들면서 작업하기에 아이디건 패스워드건 다 뒤저보더란..)
 ...
 * 복기 후 보이는 것들
 * - 자료구조 -_-;;
 * - 지나지게 낮아진 자존감 (뭘 해도 잘할 수 있는데 그리 이야기 못한 느낌?, 이 회사에 와서 자존감만 낮아진건가?)
 * - 다른곳에 가서 무얼 할 수 있는지?
 *   > 내가 잘하는건 베이스쪽 설계를 그려 그거 기반으로 공통적인 화면을 구성하게 할 수 있는 점 그걸로 인한 생산성 향상 이를 이용하는 개발자는 비즈니스 로직
 *     개발에 집중해 좀더 좋은 코드를 생산해 내도록 해줄수 있음
 * - test case 를 하고는 있지만 표출하지 않는 점?
 *
